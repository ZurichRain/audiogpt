...
import torch
from Audiogpt_dev.model.AudiogptBase import AudioGPTLlamaForCausalLM
from transformers import AutoTokenizer
from Audiogpt_dev.data.utils import *
from Audiogpt_dev.config.configs import dataConfig
...
def load_model(model_path, audio_decoder_model_config=None, audio_decoder_config=None):

    tokenizer = AutoTokenizer.from_pretrained(model_path)
    model = AudioGPTLlamaForCausalLM.from_pretrained(model_path, torch_dtype=torch.float16, low_cpu_mem_usage=True).to("cuda")
    model.init_audio_layer(audio_decoder_config,audio_decoder_model_config)
    model.init_audio_config(data_config)
    
    tokenizer.add_tokens([DEFAULT_AUDIO_PATCH_TOKEN], special_tokens=True)
    tokenizer.add_tokens([DEFAULT_AUDIO_DECODER_START_TOKEN], special_tokens=True)
    tokenizer.add_tokens([DEFAULT_AUDIO_DECODER_PAD], special_tokens=True)
    tokenizer.add_tokens([DEFAULT_AUDIO_DECODER_END_TOKEN], special_tokens=True)
    
    # print(tokenizer(DEFAULT_AUDIO_DECODER_START_TOKEN))
...
def get_input_for_model(config_data, prompt, tokenizer, wav_path=None):
    ...
    if wav_path is not None:
        audios=[[spec.transpose(0, 1).to("cuda", dtype=torch.float16)]]
...
wav_path = "/data/hypertext/sharpwang/TTS/Audiogpt_dev/inference/5290-26685-0000.wav"
prompt = "你能将这段语音的主要信息陈述出来吗？<audio>。"
# 5290-26685-0000
model_path = "/data/hypertext/sharpwang/TTS/Audiogpt_dev/checkpoints/Audiogpt-7b-stage1-paimeng_v1_epoch_10_lr_2e-5_seed_1226_sep_by_->_gen"
...
audio_decoder_model_config = config_dict["audio_decoder_model_config"]
audio_decoder_config = dataConfig(config_dict["audio_decoder_config"])


model, tokenizer = load_model(model_path,audio_decoder_config,audio_decoder_model_config)
# model.init_audio_layer(audio_decoder_config,audio_decoder_model_config)

# Dataset

input_ids, audios, audio_lens, audio_start = get_input_for_model(data_config, prompt, tokenizer, wav_path=wav_path)

ans = model.inference(
    input_ids=input_ids,
    tokenizer = tokenizer,
    input_audios = audios,
    input_audio_lens = audio_lens,
    input_audio_start = audio_start,
    temperature = 0
)
